...
/*
* if this state is new (not recorded in qMap), retrieve q values from Conway_V2_base
* get min q, max q and actions are all from qMaps
*
* the Conway_V2_base reads a double array
*
* */
...
public class GomokuAI_NN extends GomokuAI_2{
    ...
    private static final int recordAmountBeforeTrain = 5000; //the amount of moves to record before training it in Conway_V2_base
    private static final int trainingEpochs = 1;
    private static final int hiddenLayerCount = 40;
    ...
    public static void main(String[] args) {
        ...
        //saveObject(new File("network_9_3p2.eg"), network2); //save network
    ...
    HashMap<Integer, Double> createActionQValueMap(double [] stateKey, int player) {
        ...
        for (int i = 0; i < stateKey.length; i++) {
            if (stateKey[i] == 0) //if the spot board is empty (will only make actions for empty places on the board)
                vals.put(i, nnOutputs[i]); // action, Q value generated by Conway_V2_base
        }
        return vals;
    }
...
//    }
    ...
    void trainNeuralNetworks(){
        //System.out.println("train Conway_V2_base");
        trainNN(1);
        trainNN(2);
    ...
    //the Conway_V2_base input is what the game board looks like. the output is the q values for each action,
    //the Conway_V2_base has GAMEBOARD_SIZE inputs and outputs
    void trainNN(int player){
        ...
        if (player == 1) {
            ...
            for (Map.Entry<String, HashMap<Integer, Double>> entry : qMap.entrySet()) {
                ...
                for (int i = 0; i < GAMEBOARD_SIZE; ++i) {
                    ...
                    //else qValues[index][i] = Double.NEGATIVE_INFINITY;
                }
                ++index;
            }
        ...
        //train player 2 Conway_V2_base
            ...
            for (Map.Entry<String, HashMap<Integer, Double>> entry : qMap2.entrySet()) {
                ...
                for (int i = 0; i < GAMEBOARD_SIZE; ++i) {
                    if (vals.get(i) != null) qValues[index][i] = vals.get(i);
                    //else qValues[index][i] = Double.POSITIVE_INFINITY;
                }
                ++index;
            }
